...
interface BuyerRevealsPageProps {
  myApprovedReveals: ConnectionRequest[];
}

export default function BuyerRevealsPage({ myApprovedReveals }: BuyerRevealsPageProps) {
  return (
    <div className="bg-white rounded-2xl border p-6 space-y-4 text-xs text-slate-800">
      ...
      <p className="text-xs text-gray-500 max-w-lg mb-4">
        Below are all real estate plot direct seller contacts where Lipa Na M-PESA escrow connection was approved by our staff desk. Feel free to arrange physical title deeds evaluation meetings. No brokers!
      </p>

      {myApprovedReveals.length === 0 ? (
        <div className="p-12 text-center text-xs text-slate-400 italic">No approved sellers registered on your ledger.</div>
      ) : (
          ...
          {myApprovedReveals.map((c) => (
            <div key={c.id} className="p-4 bg-green-50/40 rounded-xl border border-green-200 flex flex-col sm:flex-row items-baseline sm:items-center justify-between gap-4 text-xs font-sans">
              <div className="min-w-0 flex-1">
                <span className="text-[10px] uppercase font-mono tracking-wider bg-green-100 text-green-700 px-2 py-0.5 rounded font-extrabold mb-1 inline-block">Direct seller unlocked</span>
                <h4 className="font-extrabold text-sm text-slate-800 tracking-wide">{c.propertyName}</h4>
                <p className="text-slate-500 mt-0.5 font-sans">Advertised Seller: <span className="font-bold">{c.landlordName}</span></p>
              </div>

              <div className="flex flex-col sm:items-end font-mono">
                <span className="text-[10px] text-gray-400 uppercase font-bold">Seller direct dialing:</span>
